这个文章将说明如何使用Cloudflare Email Routing 免费发送邮件

## 前置条件

需要你的cloudflare 已经接管了你的域名



## 创建 email work

在cloudflare下的workers-and-pages 中点击新建worker，然后根据 点击创建woker，点击部署，然后再次编辑你的worker 的代码，输入下面代码后，点击部署 代码如下：
...
如果你之前用过cloudflare域名下的电子邮件功能，那么你需要这样配置dns

```
TXT    yourDomain       v=spf1 include:_spf.mx.cloudflare.net include:relay.mailchannels.net ~all
...
```
TXT    yourDomain       v=spf1  include:relay.mailchannels.net ~all
TXT    _mailchannels     v=mc1 cfid=xxxxx.workers.dev
```

注意：cfid是你登录cloudflare  dashboard主页的Workers 和 Pages概览，右下角的子域/subdomain
